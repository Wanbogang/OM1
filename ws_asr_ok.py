...
def read_wav_b64(path: str) -> str:
    with open_wav(path) as wf:
        ...
        if sr != 16000 or ch != 1 or sw != 2:
            print(
                f"Warning: WAV ideal 16kHz/mono/16-bit. Sekarang: {sr}Hz, ch={ch}, {8 * sw}bit",
                flush=True,
            )
...
async def main():
    ...
    async with websockets.connect(
        ...
        while True:
            ...
            try:
                ...
                if "transcript" in obj:
                    ...
                    "alternatives" in obj
                    and isinstance(obj["alternatives"], list)
                    and obj["alternatives"]
                ):
        ...
        if not got_any:
            print(
                "Tidak ada respons selain handshake. Coba durasi lebih lama atau cek log server."
            )
