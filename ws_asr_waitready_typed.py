...
async def main():
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python -u ws_asr_waitready_typed.py tools/asr-eval/en/001.wav en-US"
        )
        sys.exit(1)
    ...
    with contextlib.closing(wave.open(wav_path, "rb")) as wf:
        ...
        async with websockets.connect(
            ...
            try:
                ...
                print(
                    f"[i] Tidak ada 'connection/ready' dalam {wait_ready_timeout}s — lanjut kirim audio (fallback)."
                )
