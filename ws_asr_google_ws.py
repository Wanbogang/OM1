...
def build_ws_url() -> str:
    ...
    if not ep:  raise RuntimeError("OM1_ASR_ENDPOINT kosong (contoh: wss://api.openmind.org/api/core/google/asr)")
    if not key: raise RuntimeError("OM_API_KEY kosong")
    sep = "&" if "?" in ep else "?"
...
async def stream_chunks(ws, wav_path: str, language: str):
    ...
    for i, payload in enumerate((
        ...
        try:
            if DEBUG: print(f"[->] flush#{i}", payload)
            await ws.send(json.dumps(payload))
            await asyncio.sleep(0.3)
        except Exception as e:
            if DEBUG: print(f"[flush#{i}] err:", e)
...
async def send_single(ws, wav_path: str, language: str):
    ...
    if DEBUG: print("[->] single", {"config": msg["config"], "audioContent": f"<base64:{len(b64)}>"})
    await ws.send(json.dumps(msg))
    # finalize ringan
    for payload in ({"type":"stop"},{"type":"end"}):
        try:
            await ws.send(json.dumps(payload)); await asyncio.sleep(0.2)
        except: pass
