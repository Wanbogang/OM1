...
SMART_ASSISTANT_ENABLED = (
    os.getenv("SMART_ASSISTANT_ENABLED", "false").lower() == "true"
)
...
# Initialize the bot (global instance)
bot = None
...
async def get_menu():
    ...
    for emoji, item in bot.emoji_processor.emoji_to_item.items():
        menu_items.append({"emoji": emoji, "name": item.name, "price": item.price})
...
async def create_order(request: OrderRequest):
    ...
    try:
        ...
        if result["success"]:
            ...
            return OrderResponse(
                success=False, message=result.get("error", "Failed to create order")
            )
...
async def list_orders(status: Optional[str] = None, limit: int = 20):
    ...
    try:
        ...
        for order in orders:
            ...
            items_summary = ", ".join(
                [f"{emoji}x{count}" for emoji, (count, _) in item_counts.items()]
            )
...
async def process_voice_command(request: VoiceCommandRequest):
    ...
    try:
        current_bot = get_bot()
        result = current_bot.process_voice_command(
            command=request.command, customer_name=request.customer_name
        )
...
async def create_smart_order(request: SmartOrderRequest):
    ...
    try:
        current_bot = get_bot()
        result = current_bot.create_order_with_smart_assistant(
            emoji_string=request.emoji_string, customer_name=request.customer_name
        )
        return {"success": True, "data": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_smart_status(order_id: str, status: StatusUpdate):
    ...
    try:
        current_bot = get_bot()
        success = current_bot.update_payment_status_smart_assistant(
            order_id=order_id, status=status.status
        )
        if success:
...
async def smart_assistant_status():
    ...
    try:
        current_bot = get_bot()
        return {
            "enabled": current_bot.smart_assistant.enabled,
            "connected": current_bot.smart_assistant.ha_url is not None,
            "ha_url": (
                current_bot.smart_assistant.ha_url
                if current_bot.smart_assistant.enabled
                else None
            ),
        }
    except Exception as e:
...
try:
    pass  # ensures /api/order_status_callback is registered
except Exception:
    pass
# --- end added by helper ---
