...
async def try_google_rest(ws, wav_path: str, language: str, tail_s: float) -> bool:
    ...
    ok = any(
        m.get("type") not in ("connection", "error")
        for m in msgs
...
async def try_root_audio_then_end(
    ws, wav_path: str, language: str, tail_s: float
) -> bool:
...
async def main():
    ...
    print(
        "[X] Keduanya belum menerima hasil. Dari log yang kamu kirim sebelumnya, ini memang terjadi jika gateway tidak mengenali bentuk payload audio."
    )
    print("    Jika masih begitu, langkah berikutnya:")
    print(
        "    - Coba WAV full (tanpa dipecah): skrip ini sudah pakai itu di mode GOOGLE_REST."
    )
    print(
        "    - Jika gateway mengharuskan field lain (mis. 'seq' atau 'requestId'), itu harus diketahui dari dokumen server."
    )
