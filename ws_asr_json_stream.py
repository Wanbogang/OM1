...
def assert_wav_ok(path):
    with wave.open(path, "rb") as w:
        ...
        if (sr, ch, sw*8) != (RATE, CHANNELS, BITS):
            raise SystemExit(f"WAV must be {RATE}Hz/{CHANNELS}ch/{BITS}bit; got {sr}Hz/{ch}ch/{sw*8}bit")
    ...
    async with websockets.connect(url, max_size=None, ping_interval=20, ping_timeout=20) as ws:
        ...
        while True:
            ...
            if not raw:
                break
            last_b64 = b64(raw)
            await ws.send(json.dumps({"requestId": request_id, "seq": seq, "audio": last_b64}))
            seq += 1

        # === Frame terakhir: tetap ada 'audio' + end ===
        if last_b64 is None:
            last_b64 = ""  # aman: tetap ada key 'audio'
        await ws.send(json.dumps({"requestId": request_id, "seq": seq, "audio": last_b64, "end": True}))
