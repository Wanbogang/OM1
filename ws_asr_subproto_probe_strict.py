...
SUBPROTOS = [None, "om1-asr", "asr", "google-asr", "speech", "asr.v1", "om1.v1", "v1", "json"]

def open_wav(p):
    w = wave.open(p,"rb")
    sr, ch, sw = w.getframerate(), w.getnchannels(), w.getsampwidth()
    assert (sr,ch,sw)==(16000,1,2), f"Need 16k/mono/16-bit, got {sr}/{ch}/{sw*8}"
    return w, sr
...
def is_result(msg: str) -> bool:
    ...
    return any(k in m for k in ["asr_reply", "result", "results", "final", "transcript", "alternatives"])
...
async def try_one(proto):
    ...
    try:
        async with websockets.connect(URL, max_size=None, subprotocols=[proto] if proto else None) as ws:
            ...
            payload = {"config":{"languageCode":"en-US","encoding":"LINEAR16","sampleRateHertz":sr,"channels":1,"bitsPerSample":16},
                       "audio": b64(frames)}
