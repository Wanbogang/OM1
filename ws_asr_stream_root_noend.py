...
async def main():
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python -u ws_asr_handshake_root.py tools/asr-eval/en/001.wav en-US"
        )
        sys.exit(1)
    ...
    with contextlib.closing(wave.open(wav_path, "rb")) as wf:
        ...
        print(
            f"CHUNK_MS    = {chunk_ms} ms, MODE=typed-start + root-audio + typed-end\n"
        )
        ...
        async with websockets.connect(
            ...
            # 1) HANDSHAKE: typed "start" (tapi tetap sertakan audio kosong!)
            start_msg = {
                "type": "start",
                "language": language,
                "sample_rate": 16000,
