...
async def main():
    if len(sys.argv) != 3:
        ...
        print("Example: python -u ws_asr_typed_stream_fixed.py tools/asr-eval/en/001.wav en-US")
        sys.exit(1)
    ...
    with contextlib.closing(wave.open(wav_path, "rb")) as wf:
        ...
        async with websockets.connect(url, ping_interval=20, ping_timeout=20, max_size=None) as ws:
            ...
            while True:
                ...
                print(f"[->] audio seq={seq} frames+={len(buf)//(2*ch)} (total ~{(seq+1)*frames_per_chunk})")
                seq += 1
