    ...
    print(
        ...
        (
            ...
            if len(str(payload)) < 200
                ...
                k: (
                    v
                    if k != "audio" and k != "data" and k != "audioContent"
                    else "<base64>"
                )
...
async def main():
    if len(sys.argv) != 3:
        ...
        print(
            "Example: python -u ws_asr_single_root.py tools/asr-eval/en/001.wav en-US"
        )
        sys.exit(1)
    ...
    for label, payload in variants:
        got = await send_once(
            url, payload, label, read_tail=float(os.getenv("ASR_READ_TAIL", "8"))
        )
    ...
    if not ok_any:
        ...
        print(
            "    2) typed_audio       -> {'type':'audio','audio':'<base64>','language':'en-US'}"
        )
        print("    3) root_data         -> {'data': '<base64>', 'language': 'en-US'}")
        print(
            "    4) root_audioContent -> {'audioContent': '<base64>', 'languageCode': 'en-US'}"
        )
        print("   Tips:")
