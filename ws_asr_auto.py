    ...
    while asyncio.get_event_loop().time() < end_at:
        try:
            msg = await asyncio.wait_for(
                ws.recv(), timeout=max(0.1, end_at - asyncio.get_event_loop().time())
            )
...
async def main():
    ...
    if not any_ok:
        print(
            "Semua protokol gagal. Periksa kembali format pesan yang diharapkan server (kunci 'audio' vs 'data' "
