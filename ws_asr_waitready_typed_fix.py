#!/usr/bin/env python3
# WS ASR client — typed JSON (start -> audio -> end)
# Catatan: beberapa gateway rewel pada urutan key; kita pastikan "audio" -> "type".
...
async def main():
    if len(sys.argv) != 3:
        print("Usage: python -u ws_asr_waitready_typed_fix.py <wav> <lang>")
        print("Example: python -u ws_asr_waitready_typed_fix.py tools/asr-eval/en/001.wav en-US")
        sys.exit(1)
    ...
    with contextlib.closing(wave.open(wav_path, "rb")) as wf:
        ...
        print(f"CHUNK_MS    = {chunk_ms} ms, MODE=typed JSON (audio-first)\n")
        ...
        async with websockets.connect(url, ping_interval=20, ping_timeout=20, max_size=None) as ws:
            ...
            try:
                ...
                print(f"[i] Tidak ada 'connection/ready' dalam {wait_ready_timeout}s — lanjut kirim audio.")

            # STREAM: pastikan 'audio' jadi key pertama (gateway naif sering cek berurutan)
            seq = 0
            t0 = time.monotonic()
            ...
            while True:
                ...
                msg["audio"] = b64
                msg["type"]  = "audio"
                # opsional kalau gateway minta bahasa di setiap chunk:
