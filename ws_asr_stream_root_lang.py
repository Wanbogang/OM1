# ws_asr_stream_root_lang.py
# Stream audio sebagai root JSON:
#   pesan#1: {"audio":"<b64>", "language":"en-US", "sample_rate":16000, "channels":1, "encoding":"LINEAR16"}
#   pesan#n: {"audio":"<b64>"}            (tanpa "type")
...
async def reader(ws):
    try:
        async for raw in ws:
            ...
            # beberapa kemungkinan bentuk hasil
...
async def stream(ws, wav_path: str, language: str):
    with open_wav(wav_path) as wf:
        ...
        while True:
            ...
            total += frames
            if DEBUG:
                print(f"[->] audio#{seq} frames+={frames} (total={total})")
            seq += 1
...
async def main():
    if len(sys.argv) < 3:
        print("Usage: python -u ws_asr_stream_root_lang.py <wav_path> <language>")
        sys.exit(2)
