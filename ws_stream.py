...
async def send_typed(ws, raw, chunk_bytes, end_type, chunk_ms, sr, ch, sw, lang):
    ...
    n_chunks = (total + chunk_bytes - 1) // chunk_bytes
    for i in range(n_chunks):
        b64 = base64.b64encode(raw[i * chunk_bytes : (i + 1) * chunk_bytes]).decode(
            "ascii"
        )
    ...
    for mode in order:
        ...
        try:
            async with websockets.connect(
                ...
                if mode == "json_root":
                    await send_json_root(
                        ws, raw, chunk_bytes, end_type, chunk_ms, lang=language
                    )
                elif mode == "binary":
                    await send_binary_no_start(ws, raw, chunk_bytes, end_type, chunk_ms)
                else:
                    await send_typed(
                        ws, raw, chunk_bytes, end_type, chunk_ms, sr, ch, sw, language
                    )
...
if __name__ == "__main__":
    ...
    if not ws_url:
        print("[!] Please set ASR_WS_URL.", file=sys.stderr)
        sys.exit(1)
